...
from algorithms.utils.buffer import HAQReplayBuffer
...
class HAQJSBSimRunner(Runner):
    ...
    def load(self):
        ...
        # policy & algorithm
        if self.algorithm_name == "hasac":
            from algorithms.hasac.sac_trainer import SACTrainer as Trainer
            from algorithms.hasac.sac_policy import SACPolicy as Policy
        else:
            raise NotImplementedError
        self.policy = Policy(self.all_args, self.num_agents if not self.use_selfplay else self.num_agents // 2, self.obs_space, self.share_obs_space, self.act_space, device=self.device)
        self.trainer = Trainer(self.all_args, device=self.device)

        # buffer
        if self.use_selfplay:
            self.buffer = HAQReplayBuffer(self.all_args, self.num_agents // 2, self.obs_space, self.share_obs_space, self.act_space)
        else:
            self.buffer = HAQReplayBuffer(self.all_args, self.num_agents, self.obs_space, self.share_obs_space, self.act_space)
        ...
        if self.use_selfplay:
            ...
            self.opponent_policy = [
                Policy(self.all_args, self.num_agents // 2, self.obs_space, self.share_obs_space, self.act_space, device=self.device)
            ...
            if self.use_eval:
                self.eval_opponent_policy = Policy(self.all_args, self.num_agents // 2, self.obs_space, self.share_obs_space, self.act_space, device=self.device)
    ...
    def collect(self, step):
        ...
        values, actions, action_log_probs, rnn_states_actor, rnn_states_critic \
            = self.policy.get_actions(self.buffer.share_obs[step],
                                      ...
                                      self.num_agents // 2 if self.use_selfplay else self.num_agents)
        ...
        if self.use_selfplay:
            ...
            for policy_idx, policy in enumerate(self.opponent_policy):
                ...
                opponent_action, opponent_rnn_states \
                    = policy.act(self.opponent_obs[env_idx],
                                self.opponent_rnn_states[env_idx],
                                self.opponent_masks[env_idx])
                temp = np.array(np.split(_t2n(opponent_action), len(env_idx)))
    ...
    def compute(self):
        self.policy.prep_rollout()
        next_values = self.policy.get_values(self.buffer.share_obs[-1],
                                            self.buffer.actions[-1],
    ...
    def eval(self, total_num_steps):
        ...
        while total_episodes < self.eval_episodes:
            ...
            self.policy.prep_rollout()
            eval_actions, eval_rnn_states = self.policy.act(eval_obs,
                                                            eval_rnn_states,
            ...
            if self.use_selfplay:
                eval_opponent_actions, eval_opponent_rnn_states \
                    = self.eval_opponent_policy.act(eval_opponent_obs,
                                                    eval_opponent_rnn_states,
                                                    eval_opponent_masks)
