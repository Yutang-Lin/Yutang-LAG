...
from algorithms.utils.buffer import HAReplayBuffer
...
class HAJSBSimRunner(Runner):
    ...
    def load(self):
        ...
        if self.algorithm_name == "happo":
            from algorithms.happo.ppo_trainer import PPOTrainer as Trainer
            from algorithms.happo.ppo_policy import PPOPolicy as Policy
        else:
        ...
        if self.use_selfplay:
            self.buffer = HAReplayBuffer(self.all_args, self.num_agents // 2, self.obs_space, self.share_obs_space, self.act_space)
        else:
            self.buffer = HAReplayBuffer(self.all_args, self.num_agents, self.obs_space, self.share_obs_space, self.act_space)
